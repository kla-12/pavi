...
// Ollama model name patterns (local models)
...
function resolveProvider(model) {
  if (typeof model === "string") {
    // Check for explicit Ollama prefix override
    if (model.startsWith("ollama:")) return "ollama";
    // Auto-detect known local model patterns
    if (OLLAMA_MODEL_PATTERNS.some(p => p.test(model))) return "ollama";
...
// Dynamically fetch local Ollama models and merge with known cloud models
...
// Start with phi3:mini as a guaranteed local model (just downloaded)
let KNOWN_MODELS = [...CLOUD_MODELS, "phi3:mini"];

app.get("/models", async (_, res) => {
  // Merge cloud models with live Ollama models
  const ollamaModels = await fetchOllamaModels();
  const allModels = [...new Set([...CLOUD_MODELS, ...ollamaModels])];
  // Update cache
  KNOWN_MODELS = allModels;
  res.json({ object: "list", data: allModels.map(id => ({
    id,
    object: "model",
    owned_by: ollamaModels.includes(id) ? "ollama" : "system",
    providers: [{ supports_tools: !ollamaModels.includes(id) }],
  })) });
});
